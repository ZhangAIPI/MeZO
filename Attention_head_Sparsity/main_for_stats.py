...
def parse_args():
    ...
    parser.add_argument(
        "--r", type=int, default=None, help="Number of experts to preserve"
    )
    parser.add_argument(
        "--calib_set",
        type=str,
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for reproduction"
    )
    parser.add_argument(
...
def main(args: Namespace):
    ...
    if args.method.endswith("_pruning"):
        ...
        save_path = osp.join(
            args.output_path,
            f'{model_name}_{args.method}_r{args.r}_{args.calib_set}_{args.n_blocks_for_stat}_{"fatt2_" if args.use_flash_attention_2 else ""}{datetime.now().strftime("%Y%m%d-%H%M%S")}',
        )
    else:
        ...
        save_path = osp.join(
            args.output_path,
            f'{model_name}_{args.method}_{args.calib_set}_{args.n_blocks_for_stat}_{"fatt2_" if args.use_flash_attention_2 else ""}{datetime.now().strftime("%Y%m%d-%H%M%S")}',
        )
    ...
    layer_output_x, layer_output_z, layer_wo = METHODS[args.method](
        model, calib_loader, args
    )

    with open(
        "/home/zhiyuan/patch_tst/global_prune/Attention_head_Sparsity/data/stats/{}.pkl".format(
            model_name
        ),
        "wb",
    ) as file:
        ...
        pkl.dump([layer_output_x, layer_output_z, layer_wo], file)
